...
@WebServlet(name="apigenre", urlPatterns = {"/apigenre", "/apigenre/*"})
public class APIGenre extends HttpServlet {
	...
	protected void doDelete(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		try {
			genreService.delete(ConnexionPSQL.getInstance(), id);
		} catch (SQLException e) {
			e.printStackTrace();
		}
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		String genreToAdd = gson.fromJson(req.getReader(), String.class);
		
		
		try {
			genreService.insert(ConnexionPSQL.getInstance(), genreToAdd);
			Util.formatResponse(resp, new Message("Le genre " + genreToAdd + " a bien été ajouté"));
		} catch (SQLException e) {
